...
# Add all variables incl. given board
for i in range(len(riddle)):
   for j in range(len(riddle[i])):
        if riddle[i][j] == 0:
            sudoku.addVariable(rownames[i] + colnames[j], [1, 2, 3, 4, 5, 6, 7, 8, 9])
        else:
            sudoku.addVariable(rownames[i] + colnames[j], [riddle[i][j]])
...
sortedSolutionValues = SortedDict(solutions[0]).values();

print(sortedSolutionValues)
